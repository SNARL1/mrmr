...
#' for survival in the years follwing introduction.
#' @examples
#' \dontrun{
...
survival_table <- function(model, by_cohort = TRUE) {
  ...
  if (by_cohort) {
    survival_summary <- survival_summary %>%
      group_by(.data$years_since_introduction, .data$iter, .data$release_date)
  } else {
    survival_summary <- survival_summary %>%
  ...
  survival_summary %>%
    ...
    summarize(lo_survival = quantile(.data$fraction_alive, .025),
